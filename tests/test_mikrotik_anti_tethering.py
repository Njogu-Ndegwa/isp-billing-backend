...
class FakeMikroTikAPI(MikroTikAPI):
    def __init__(self, *, hotspots=None, mangle_rules=None, filter_rules=None):
        self.connected = True
...
def test_enable_anti_tethering_orders_drop_before_fasttrack_and_clamps_return_ttl():
    api = FakeMikroTikAPI(
        mangle_rules=[
    ...
    assert result["mode"] == "ttl-marker-plus-return-ttl-clamp"
...
def test_disable_anti_tethering_removes_only_managed_rules():
    ...
    assert result == {"success": True, "removed_mangle": 1, "removed_filter": 1}
    assert api.commands == [
