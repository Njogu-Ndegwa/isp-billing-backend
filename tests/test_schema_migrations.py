...
def test_mpesa_hot_path_index_is_concurrent_bounded_and_wired_to_startup():
    ...
    assert "create index concurrently if not exists ix_mpesa_txn_status_created" in normalized
    assert "on public.mpesa_transactions (status, created_at asc)" in normalized
    assert "create index concurrently if not exists ix_mpesa_txn_customer_created" in normalized
    assert "on public.mpesa_transactions (customer_id, created_at desc)" in normalized
