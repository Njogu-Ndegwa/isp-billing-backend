...
async def test_apply_lock_serializes_same_reseller_on_postgres():
    class FakeDialect:
        name = "postgresql"
    ...
    db = FakeSession()
    await _lock_reseller_outage_runs(db, 42)

    assert "pg_advisory_xact_lock" in db.statement
    assert db.params["reseller_id"] == 42
...
async def test_apply_extends_expiry_and_writes_audit_trail(db):
    ...
    assert result["customers"][0]["expiry"] == expiry_before.isoformat()
    assert result["customers"][0]["new_expiry"] == (
        expiry_before + timedelta(seconds=outage_seconds)
    ).isoformat()
