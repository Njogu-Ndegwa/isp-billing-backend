...
class ProvisioningToken(Base):
    ...
    # Set only for RouterOS 6 tokens created while PROVISION_MGMT_TO_HETZNER:
    ...
    # Management tunnel chosen at token creation: "wireguard" (RouterOS 7,
    # wg-hz to Hetzner wg2) or "sstp" (RouterOS 6, sstp-hetzner) when
    # PROVISION_MGMT_TO_HETZNER was on -- the router's ONLY tunnel. NULL = the
    # legacy layout (AWS primary + Hetzner insurance/standby). Copied to
    # routers.management_tunnel on /complete.
    management_tunnel = Column(String(20), nullable=True)
