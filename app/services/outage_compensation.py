...
* Currently-active customers are credited by default. When the reseller opts
  in, customers whose paid time expired during or after the outage are revived
  from ``now`` and re-provisioned after the DB transaction commits. Router I/O
  never happens while this service holds a pooled connection.
...
# Two concurrent apply requests for one reseller must not both pass the
# overlap check before either inserts its audit row. PostgreSQL transaction
# advisory locks serialize only this operation and release automatically on
# commit/rollback; SQLite remains a no-op for the test suite.
_OUTAGE_LOCK_NAMESPACE = 1330992205


async def _lock_reseller_outage_runs(db: AsyncSession, reseller_id: int) -> None:
    bind = db.get_bind()
    if bind.dialect.name != "postgresql":
        return
    await db.execute(
        text("SELECT pg_advisory_xact_lock(:namespace, :reseller_id)"),
        {"namespace": _OUTAGE_LOCK_NAMESPACE, "reseller_id": reseller_id},
    )
...
async def apply_outage_compensation(
    ...
    *,
    ...
    await _lock_reseller_outage_runs(db, reseller_id)
    overlaps = await _overlapping_runs(
        db, reseller_id, router_id_list, outage_start, outage_end
    )
    ...
    for customer in affected:
        ...
        if credit <= 0:
            continue
        credited_row = _customer_row(customer, credit)
        ...
        credited_rows.append(credited_row)
    ...
    if include_expired:
        for customer in skipped_expired:
            ...
            if credit <= 0:
                continue
            revived_row = _revival_row(customer, credit, now)
            expiry_before = customer.expiry
            ...
            revived_rows.append(revived_row)
